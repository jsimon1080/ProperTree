...
def _update_tex(queue, tex_url=None, tex_path=None):
    ...
    if os.path.exists(file_path):
        ...
        return queue.put({"exception": "Could not locate update_check.py.", "error": "Missing Required Files"})
    if tex_url:
        args.extend(["-u", tex_url])
    proc = subprocess.Popen(args, stdout=subprocess.PIPE)
    o, e = proc.communicate()
    if sys.version_info >= (3, 0):
        o = o.decode("utf-8")
...
class ProperTree:
    def __init__(self, plists=[]):
        ...
        self.plist_type_menu = tk_or_ttk.OptionMenu(
            ...
            *self.get_option_menu_list(self.allowed_types),
        ...
        self.data_type_menu = tk_or_ttk.OptionMenu(
            ...
            *self.get_option_menu_list(self.allowed_data),
        ...
        self.int_type_menu = tk_or_ttk.OptionMenu(
            ...
            *self.get_option_menu_list(self.allowed_int),
        ...
        self.bool_type_menu = tk_or_ttk.OptionMenu(
            ...
            *self.get_option_menu_list(self.allowed_bool),
        ...
        self.snapshot_menu = tk_or_ttk.OptionMenu(
            self.settings_window, self.snapshot_string, "Auto-detect", command=self.change_snapshot_version
        )
        ...
        self.schema_check = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Force Update Snapshot Schema",
            variable=self.force_schema,
            command=self.schema_command,
        )
        self.schema_check.grid(row=12, column=0, columnspan=3, sticky="w", padx=10)
        self.mod_check = tk.IntVar()
        self.enable_mod_check = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Warn If Files Are Externally Modified",
            variable=self.mod_check,
            command=self.mod_check_command,
        )
        self.enable_mod_check.grid(row=13, column=0, columnspan=3, stick="w", padx=10)
        self.first_check = tk.IntVar()
        self.enable_first_check = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Enter Edits Values Before Keys Where Possible",
            variable=self.first_check,
            command=self.first_check_command,
        )
        self.enable_first_check.grid(row=14, column=0, columnspan=3, stick="w", padx=10)
        self.enable_drag_and_drop = tk.BooleanVar()
        self.toggle_drag_drop = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Enable Row Drag & Drop",
            variable=self.enable_drag_and_drop,
            command=self.drag_drop_command,
        )
        ...
        self.drag_scale = tk_or_ttk.Scale(
            self.settings_window, from_=1, to=100, orient=tk.HORIZONTAL, command=self.scale_command
        )
        # Try to hide the value if using tk - will throw an exception in ttk
        try:
            self.drag_scale.configure(showvalue=False)
        except tk.TclError:
            pass
        ...
        self.op_scale = tk_or_ttk.Scale(
            self.settings_window, from_=25, to=100, orient=tk.HORIZONTAL, command=self.update_opacity
        )
        # Try to hide the value if using tk - will throw an exception in ttk
        try:
            self.op_scale.configure(showvalue=False)
        except tk.TclError:
            pass
        ...
        self.ig_bg = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Header Text Ignores BG Color",
            variable=self.ig_bg_check,
            command=self.check_ig_bg_command,
        )
        self.ig_bg.grid(row=6, column=4, sticky="w", padx=10)
        self.bg_inv_check = tk.IntVar()
        self.bg_inv = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Invert Header Text Color",
            variable=self.bg_inv_check,
            command=self.check_bg_invert_command,
        )
        self.bg_inv.grid(row=6, column=5, columnspan=2, sticky="w", padx=10)
        self.r1_inv_check = tk.IntVar()
        self.r1_inv = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Invert Row #1 Text Color",
            variable=self.r1_inv_check,
            command=self.check_r1_invert_command,
        )
        self.r1_inv.grid(row=7, column=5, columnspan=2, sticky="w", padx=10)
        self.r2_inv_check = tk.IntVar()
        self.r2_inv = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Invert Row #2 Text Color",
            variable=self.r2_inv_check,
            command=self.check_r2_invert_command,
        )
        self.r2_inv.grid(row=8, column=5, columnspan=2, sticky="w", padx=10)
        self.hl_inv_check = tk.IntVar()
        self.hl_inv = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Invert Highlight Text Color",
            variable=self.hl_inv_check,
            command=self.check_hl_invert_command,
        )
        ...
        self.font_check = tk_or_ttk.Checkbutton(
        ...
        self.font_custom_check = tk_or_ttk.Checkbutton(
            self.settings_window, text="Use Custom Font", variable=self.font_var, command=self.font_select
        )
        self.font_custom = ttk.Combobox(
            self.settings_window, state="readonly", textvariable=self.font_family, values=sorted(families())
        )
        ...
        default_high = tk_or_ttk.Button(
            self.settings_window, text="Highlight Color", command=lambda: self.swap_colors("highlight")
        )
        default_high.grid(row=14, column=4, sticky="we", padx=10)
        default_light = tk_or_ttk.Button(
            self.settings_window, text="Light Mode Colors", command=lambda: self.swap_colors("light")
        )
        default_light.grid(row=13, column=5, columnspan=2, sticky="we", padx=10)
        default_dark = tk_or_ttk.Button(
            self.settings_window, text="Dark Mode Colors", command=lambda: self.swap_colors("dark")
        )
        ...
        self.update_check = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Check For Updates At Start",
            variable=self.update_int,
        ...
        self.notify_once_check = tk_or_ttk.Checkbutton(
            self.settings_window,
            text="Only Notify Once Per Version",
            variable=self.notify_once_int,
            command=self.notify_once,
        )
        self.notify_once_check.grid(row=20, column=0, sticky="w", padx=10, pady=(0, 10))
        self.update_button = tk_or_ttk.Button(
            self.settings_window, text="Check Now", command=lambda: self.check_for_updates(user_initiated=True)
        )
        self.update_button.grid(row=20, column=1, columnspan=2, sticky="w", padx=10, pady=(0, 10))
        self.tex_button = tk_or_ttk.Button(
            self.settings_window, text="Get Configuration.tex", command=self.get_latest_tex
        )
        self.tex_button.grid(row=20, column=4, sticky="we", padx=10, pady=(0, 10))
        reset_settings = tk_or_ttk.Button(
            self.settings_window, text="Restore All Defaults", command=self.reset_settings
        )
        ...
        f_option = tk_or_ttk.OptionMenu(
            self.tk, self.f_title, *self.get_option_menu_list(self.allowed_conv), command=self.change_from_type
        )
        t_option = tk_or_ttk.OptionMenu(
            self.tk, self.t_title, *self.get_option_menu_list(self.allowed_conv), command=self.change_to_type
        )
        ...
        if str(sys.platform) == "darwin":
            ...
            file_menu.add_command(
                label="Strip Surrounding Whitespace from Keys & Values (Cmd+K)",
    ...
    def _clipboard_append(self, clipboard_string=None):
        ...
        if clipboard_string:
            ...
            clipboard_string = ""  # Ensure we have at least an empty string
    ...
    def check_dark_mode(self):
        ...
        if check_dark != self.use_dark and any(
            ...
            if color_check:  # We have something to animate
                color_list = []
                from_dict, to_dict = (
                    (self.default_dark, self.default_light)
                    if self.use_dark
                ...
                for name in color_check:
                    ...
                    color_list.append((name, from_dict[name], to_dict[name]))
        ...
        for window in windows:
            try:
                ...
                set_window_attribute(
                    hwnd_inst, DWMWA_USE_IMMERSIVE_DARK_MODE, ctypes.byref(value), ctypes.sizeof(value)
                )
                set_window_attribute(
                    hwnd_inst, DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1, ctypes.byref(value), ctypes.sizeof(value)
                )
                # Update the Window size to ensure the changes happen
                for x in (1, -1):
                    window.geometry("{}x{}".format(window.winfo_width() + x, window.winfo_height() + x))
                    window.update()
            except:
    ...
    def color_animate(self, colors, step=1, steps=5, delay=35):
        for name, start, end in colors:
            ...
            result = "#{}{}{}".format(hex(r_now)[2:].upper(), hex(g_now)[2:].upper(), hex(b_now)[2:].upper())
            self.settings[name] = result
        # Update the windows
        if step < steps:
            self.update_canvases()
            self.tk.after(delay, lambda: self.color_animate(colors, step=step + 1, steps=steps, delay=delay))
        else:
    ...
    def should_set_header_text(self):
        ...
        if str(sys.platform) != "darwin":
            return True  # Don't make changes on Windows/Linux
        try:
        ...
        if self.is_checking_for_updates:  # Already checking
            if user_initiated:
                ...
                mb.showerror(
                    ...
                    "An update check is already in progress.  If you consistently get this error when manually checking for updates - it may indicate a netowrk issue.",
                )
                self.lift_window()
            return
        self.is_checking_for_updates = True  # Lock out other update checks
        self.update_button.configure(
            state="disabled", text="Checking... ({})".format(self.version.get("version", "?.?.?"))
        )
    ...
    def reset_update_button(self):
        self.update_button.configure(state="normal", text="Check Now ({})".format(self.version.get("version", "?.?.?")))
    ...
    def reset_tex_button(self, version=None):
        ...
        self.tex_button.configure(
            state="normal", text="Get Configuration.tex{}".format(" ({})".format(tex_version) if tex_version else "")
        )

    def get_latest_tex(self):
        tex_version = self.get_tex_version()
        self.tex_button.configure(
            state="disabled", text="Downloading...{}".format(" ({})".format(tex_version) if tex_version else "")
        )
        # We'll leverage multiprocessing to avoid UI locks if the update checks take too long
    ...
    def handle_keypress(self, event, generate=True):
        if event.state & 0x2 and event.keysym != "Caps_Lock":
            ...
            sequence = "<{}{}Key-{}>".format("-".join(s), "-" if s else "", keysym)
    ...
    def update_opacity(self, event=None):
        ...
        self.op_perc.configure(text="{}".format(str(int(self.op_scale.get())).rjust(3)))
    ...
    def update_canvases(self):
        ...
        color_1 = "".join(
            [
                x
                for x in self.settings.get("alternating_color_1", default_color["alternating_color_1"])
                if x.lower() in "0123456789abcdef"
            ]
        )
        color_2 = "".join(
            [
                x
                for x in self.settings.get("alternating_color_2", default_color["alternating_color_2"])
                if x.lower() in "0123456789abcdef"
            ]
        )
        color_h = "".join(
            [
                x
                for x in self.settings.get("highlight_color", default_color["highlight_color"])
                if x.lower() in "0123456789abcdef"
            ]
        )
        color_b = "".join(
            [
                x
                for x in self.settings.get("background_color", default_color["background_color"])
                if x.lower() in "0123456789abcdef"
            ]
        )
    ...
    def update_settings(self):
        ...
        snapshot_versions = [
            (
                "{} -> {}".format(x["min_version"], x.get("max_version", "Current"))
                if x["min_version"] != x.get("max_version", "Current")
                else x["min_version"]
            )
            for x in self.snapshot_data
            if "min_version" in x and len(x["min_version"])
        ]
        snapshot_choices = ["Auto-detect", "Latest"] + sorted(snapshot_versions, reverse=True)
        for choice in snapshot_choices:
            self.snapshot_menu["menu"].add_command(
                label=choice, command=tk._setit(self.snapshot_string, choice, self.change_snapshot_version)
            )
    ...
    def compare_paths(self, check, path):
        if not isinstance(path, (str, unicode, list)):
            return False
    ...
    def normpath_recents(self):
        ...
        for path in normalized:
            if self.compare_paths(path, new_paths):
    ...
    def update_recents(self):
        ...
        targets = (
            [self]
            if str(sys.platform) == "darwin"
            else [w for w in self.stackorder(self.tk) if not w in self.default_windows]
        )
        for target in targets:
            self.update_recents_for_target(target)

    def update_recents_for_target(self, target):
        if not hasattr(target, "recent_menu"):
            ...
            for arg in args:
                ...
                existing_window = next(
                    (
                        window
                        for window in windows
                        if not window in self.default_windows and window.current_plist == arg
                    ),
                    None,
                )
                if existing_window:
                ...
                if (
                    len(windows) == 1
                    and windows[0] == self.start_window
                    and windows[0].edited == False
                    and windows[0].current_plist is None
                ):
    ...
    def duplicate_plist(self, event=None):
        ...
        if os.name == "nt":
            self.set_win_titlebar(windows=new_window)
        ...
        if str(sys.platform) != "darwin":
            self.update_recents_for_target(new_window)
    ...
    def new_plist(self, event=None):
        ...
        # Update the Open Recent menu
        if str(sys.platform) != "darwin":
            self.update_recents_for_target(window)
        # Ensure the window titlebar color is updated
        if os.name == "nt":
            self.set_win_titlebar(windows=window)
    ...
    def pre_open_with_path(self, path, current_window=None):
        if not path:
            return  # Hmmm... shouldn't happen, but just in case
        path = os.path.abspath(os.path.expanduser(path))
        windows = self.stackorder(self.tk)
        if (
    ...
    def open_plist_with_path(self, event=None, path=None, current_window=None):
        ...
        try:
            ...
            self.tk.bell()
            mb.showerror(
                "An Error Occurred While Opening {}".format(os.path.basename(path)), str(e)
        ...
        # Ensure the window titlebar color is updated
        if os.name == "nt":
            self.set_win_titlebar(windows=current_window)
    ...
    def lift_window(self, window=None, deiconify=False):
        if window is None:
            ...
            if windows:  # Get the last window we saw
                window = windows[-1]
        if window is None:
            return  # No windows in the stack order?
    ...
    def quit(self, event_or_signum=None, frame=None):
        if self.is_quitting:
            return  # Already quitting - don't try to do this twice at once
