...
class EntryPlus(ttk.Entry):
        ...
        try:
            ...
            if index == start:
                if selection_type == "word":
                    ...
                    closest_gap = get_bounds("tcl_wordBreakBefore", self.get(), closest_gap)
                start = closest_gap
            else:
                if selection_type == "word":
                    # Select the whole word
                    word = self.get()
                    closest_gap = get_bounds("tcl_wordBreakAfter", word, closest_gap, len(word))
                end = closest_gap
            # Set our selection
    ...
    def select_left_right(self, amount=-1):
        ...
        if (
            ...
            or amount == 0
            or (amount < 0 and index == 0)
            or (amount > 0 and index == self.index(tk.END))
        ):
            return "break"
    ...
    def goto_left_right(self, left=True):
        try:
            target = self.index(tk.SEL_FIRST if left else tk.SEL_LAST)
...
class EntryPopup(EntryPlus):
    ...
    def focus_out(self, event=None):
        if self.confirming:
            return  # Don't do anything if we're still confirming
        ...
        if self.column == "#0":
            ...
            for child in self.parent.get_children(parent):
                ...
                if text == self.parent.item(child, "text"):
                    ...
                    if event:
                        self.bell()  # Only bell when we have a real event (i.e. return was pressed)
                    if no_prompt or not mb.askyesno(
                        "Invalid Key Name",
                        "That key name already exists in that dict.\n\nWould you like to keep editing?",
                        parent=self.parent,
                    ):
            ...
            # Add to undo stack
            self.master.add_undo(
                {"type": "edit", "cell": self.cell, "text": self.parent.item(self.cell, "text"), "values": original}
            )
...
"""
        ...
        self.safe_path_length = (
            128  # OC_STORAGE_SAFE_PATH_MAX from Include/Acidanthera/Library/OcStorageLib.h in OpenCorePkg
        )
        ...
        self.root_type_menu.add_command(
            label="Dictionary", command=lambda: self.change_type(self.menu_code + " Dictionary")
        )
        ...
        self.style.layout(
            self.style_name + ".Heading",
            [
                ("Corp.Treeheading.cell", {"sticky": "nswe"}),
                (
                    "Corp.Treeheading.border",
                    {
                        "sticky": "nswe",
                        "children": [
                            (
                                "Corp.Treeheading.padding",
                                {
                                    "sticky": "nswe",
                                    "children": [
        ...
        self._tree = ttk.Treeview(
            self._tree_frame, columns=("Type", "Value"), selectmode="browse", style=self.style_name
        )
        ...
        if not str(sys.platform) == "darwin":
            ...
            file_menu.add_command(
                label="Convert Window",
                command=lambda: self.controller.show_window(self.controller.tk),
                accelerator="Ctrl+T",
            )
            ...
            file_menu.add_command(
                label="Strip Surrounding Whitespace from Keys & Values",
                command=lambda: self.strip_whitespace(keys=True, values=True),
                accelerator="Ctrl+K",
            )
            file_menu.add_separator()
            file_menu.add_command(
                label="Settings",
                command=lambda: self.controller.show_window(self.controller.settings_window),
                accelerator="Ctrl+,",
            )
            file_menu.add_separator()
            file_menu.add_command(label="Toggle Find/Replace Pane", command=self.hide_show_find, accelerator="Ctrl+F")
            file_menu.add_command(
                label="Toggle Plist/Data/Int/Bool Type Pane", command=self.hide_show_type, accelerator="Ctrl+P"
            )
            file_menu.add_separator()
        ...
        self.plist_type_menu = tk_or_ttk.OptionMenu(
            self.display_frame,
            self.plist_type_string,
            *self.controller.get_option_menu_list(self.controller.allowed_types),
        ...
        self.data_type_menu = tk_or_ttk.OptionMenu(
            self.display_frame,
            self.data_type_string,
            *self.controller.get_option_menu_list(self.controller.allowed_data),
        ...
        self.int_type_menu = tk_or_ttk.OptionMenu(
            self.display_frame,
            self.int_type_string,
            *self.controller.get_option_menu_list(self.controller.allowed_int),
        ...
        self.bool_type_menu = tk_or_ttk.OptionMenu(
            self.display_frame,
            self.bool_type_string,
            *self.controller.get_option_menu_list(self.controller.allowed_bool),
            command=self.change_bool_type,
        )
        ...
        f_option = tk_or_ttk.OptionMenu(
            self.find_frame,
            self.f_title,
            *self.controller.get_option_menu_list(self.f_options),
            command=self.change_find_type,
        )
        ...
        self.seq = [
            ...
            for i, x in enumerate(
                [
                    ...
                    [3200, 3552, 3808, 3520],
                    [6400, 7104, 7616, 7040],
                    [13824, 12928, 13056, 14848],
                    [29184, 26880, 26368, 26624, 29696],
                    [55296, 51712, 52224, 59392],
                    [116736, 107520, 105472, 106496, 118784],
                    [200704],
                    [397312],
                ],
                start=3,
    ...
    def quick_search(self, event=None):
        ...
        if self.controller.handle_keypress(event) == "break":
            ...
            return "break"
        try:
            if event.keysym.lower() == self.seq[self.ind]:
                self.ind += 1
            elif event.keysym.lower() == self.seq[0]:
                self.ind = {1: 1, 2: 2}.get(self.ind, 1)
            else:
                self.ind = 0
        except Exception:
            self.ind = 0
        if self.ind >= len(self.seq):
            try:
                self.m1, self.m2 = (
                    base64.b64decode("".join(b[0 + i : 5 + i][::-1] for i in range(0, len(b), 5))).decode()
    ...
    def _get_menu_commands(self, menu=None, label=False):
        if not menu:
            return []
        return [
            menu.entrycget(i, "label") if label else i
    ...
    def set_font_size(self):
        self.font["size"] = (
            self.controller.font_string.get()
            if self.controller.custom_font.get()
            else self.controller.default_font["size"]
        )
        self.style.configure(
            self.style_name,
            font=self.font,
            rowheight=int(
                math.ceil(self.font.metrics()["linespace"] * (1.125 if str(sys.platform) == "darwin" else 1.3))
            ),
        )
    ...
    def window_resize(self, event=None, obj=None):
        ...
        if not event or not obj:
            return
        if self.winfo_height() == self.previous_height and self.winfo_width() == self.previous_width:
            return
    ...
    def b_false(self, lower=False):
        return (
            self.bool_type_string.get().split("/")[-1].lower() if lower else self.bool_type_string.get().split("/")[-1]
        )
    ...
    def set_find_type_by_index(self, index=None, zero_based=False):
        if not isinstance(index, (int, long)):
            ...
            try:
                index = int(getattr(index, "keysym", None).replace("KP_", ""))
            except:
                return  # Borked value
        if not zero_based:
            index -= 1  # original index started at 1, normalize to 0-based
        if index < 0 or index >= len(self.f_options):
            return  # Out of range
    ...
    def cycle_find_type(self, event=None):
        ...
        increment = (
            event if isinstance(event, bool) else {"Up": False, "Down": True}.get(getattr(event, "keysym", None))
        )
        if increment is None:
            return  # Not sure why this was fired?
        # Set our type to the next in the list
        value = self.f_title.get()
        try:
            curr, end = self.f_options.index(value), len(self.f_options)
        except:
            return  # Menu is janked?
        ...
        if value_type == "data":
            ...
            if not 0 <= value < 1 << 32:
                return (
                    ...
                    "UIDs cannot not be negative, and must be less than 2**32 (4294967296)",
                )
            value = str(value)
        return (True, value)
    ...
    def do_replace(self, node, find, new_text):
        ...
        if find_type == "string":
            ...
            if self.data_type_string.get().lower() == "hex":
                ...
                values[1] = (
                    values[1].upper().replace(" ", "").replace("<", "").replace(">", "").upper().replace(find, new_text)
                )
    ...
    def replace(self, event=None):
        ...
        if find == repl:
            ...
            mb.showerror(
                ...
                "The find and replace values are the same.  Nothing to do.",
                parent=self,
            )
            return
        ...
        if replace_all:
            ...
            if not len(matches):
                ...
                self.bell()
                mb.showerror(
                    "No Replaceable Matches Found",
                    '"{}" did not match any {} fields in the current plist.'.format(find, self.find_type.lower()),
                    parent=self,
                )
                return
        ...
        for i, x in enumerate(matches):
            ...
            if self.do_replace(x[1], find, repl):
                replacements.append({"type": "edit", "cell": x[1], "text": name, "values": values})
    ...
    def is_match(self, node, text):
        ...
        if node_type == "data":
            ...
            if self.data_type_string.get().lower() == "hex":
                if (
                    text.replace(" ", "").replace("<", "").replace(">", "").upper()
                    in value.replace(" ", "").replace("<", "").replace(">", "").upper()
                ):
                    # Got a match!
                    return True
            else:
    ...
    def find_prev(self, event=None):
        ...
        if not len(matches):
            ...
            mb.showerror(
                "No Matches Found",
                '"{}" did not match any {} fields in the current plist.'.format(type_check[1], self.find_type.lower()),
                parent=self,
            )
            return None
    ...
    def find_next(self, event=None, replacing=False):
        ...
        if not len(matches):
            ...
            if not replacing:
                ...
                mb.showerror(
                    "No Matches Found",
                    '"{}" did not match any {} fields in the current plist.'.format(
                        type_check[1], self.find_type.lower()
                    ),
                    parent=self,
                )
            return None
    ...
    def reload_from_disk(self, event=None):
        ...
        if self.edited:
            self.bell()
            if not mb.askyesno(
                "Unsaved Changes", "Any unsaved changes will be lost when reloading from disk. Continue?", parent=self
            ):
                return
        ...
        try:
            ...
            self.bell()
            mb.showerror(
                "An Error Occurred While Opening {}".format(os.path.basename(self.current_plist)), repr(e), parent=self
            )
            return
    ...
    def check_path_length(self, item, prefix=""):
        ...
        if isinstance(item, dict):
            ...
            for key in item:
                if "path" in key.lower() and isinstance(item[key], basestring):
                    if key.lower() in ("executablepath", "plistpath") and isinstance(
                        item.get("BundlePath"), basestring
                    ):
        ...
        return [
            (item, name, paths_too_long)
        ]  # Return a list containing a tuple of the original item, and which paths are too long
    ...
    def oc_snapshot(self, event=None, clean=False):
        ...
        def check_folders(path):
            ...
            return_dict["missing"] = [
                return_dict[x] for x in ("oc_acpi", "oc_drivers", "oc_kexts") if not os.path.isdir(return_dict[x])
            ]
            return return_dict
        ...
        for snap in self.controller.snapshot_data:
            ...
            if len(oc_hash) and (oc_hash in snap.get("release_hashes", []) or oc_hash in snap.get("debug_hashes", [])):
                ...
                if user_snap.lower() == "auto-detect":
                    select_snap = snap
            # Save the snap that matches the user's choice too if not Latest or Auto-detect
            if (
                user_snap.lower() not in ("auto-detect", "latest")
                and user_snap >= snap.get("min_version", "0.0.0")
                and snap.get("min_version", "0.0.0") > select_snap.get("min_version", "0.0.0")
            ):
                select_snap = snap
                ...
                if not path in target_path:
                    ...
                    if not isinstance(target_path[path], path_type if i >= len(path_list) else dict):
                        ...
                        incorrect_types.append(
                            "{}: Expected {}, got {}".format(" -> ".join(path_list[:i]), exp_type, got_type)
                        )
                        target_path[path] = use_type()
        ...
        if missing_paths or incorrect_types:
            ...
            if missing_paths:
                ...
                error_list.extend(["The following entries are missing:\n"] + missing_paths + [""])
            if incorrect_types:
                title.append("Incorrect")
                verb.append("fix incorrect types")
                error_list.extend(["The following types are incorrect:\n"] + incorrect_types + [""])
        ...
        for aml in sorted(new_acpi, key=lambda x: x.lower()):
            ...
            new_aml_entry = {"Comment": os.path.basename(aml), "Enabled": True, "Path": aml}
        ...
        if len(acpi_duplicates):
            if mb.askyesno(
                "Duplicate ACPI Paths",
                "Disable the following ACPI entries with duplicate Paths?\n\n{}".format("\n".join(acpi_duplicates)),
                parent=self,
            ):
        ...
        for path, subdirs, files in os.walk(oc_kexts):
            for name in sorted(subdirs, key=lambda x: x.lower()):
                ...
                try:
                    ...
                    if not "CFBundleIdentifier" in info_plist or not isinstance(
                        info_plist["CFBundleIdentifier"], basestring
                    ):
                        omitted_kexts.append(name)
                    ...
                    if info_plist.get("CFBundleExecutable", None):
                        if not os.path.exists(
                            os.path.join(path, name, "Contents", "MacOS", info_plist["CFBundleExecutable"])
                        ):
                            omitted_kexts.append(name)
        ...
        original_kexts = [
            x for x in kexts if isinstance(x, dict) and x.get("BundlePath", "") in bundle_list
        ...
        for x in new_kexts:
            ...
            parents = [
                ...
                if y[1].get("cfbi", None) in info.get("osbl", [])
            ]
            children = [
                next((z for z in new_kexts if z.get("BundlePath", "") == y[0].get("BundlePath", "")), [])
                for y in kext_list
                if info.get("cfbi", None) in y[1].get("osbl", [])
            ]
            unordered_kexts.append({"kext": x, "parents": parents})
        ordered_kexts = []
        disabled_parents = []
        cyclic_kexts = []
            ...
            if len(kext["parents"]):
                ...
                if kext["kext"].get("Enabled"):
                    for p in kext["parents"]:
                        p_cf = p[1].get("cfbi")
                        if not p_cf:
                            continue  # Broken - can't check
                        if p_cf in enabled_parents:
                            continue  # Already have an enabled copy
        ...
        if len(rearranged):
            if not mb.askyesno(
                "Incorrect Kext Load Order",
                "Correct the following kext load inheritance issues?\n\n{}".format("\n".join(rearranged)),
                parent=self,
            ):
                ordered_kexts = original_kexts  # We didn't want to update it
        if len(disabled_parents):
            if mb.askyesno(
                "Disabled Parent Kexts",
                "Enable the following disabled parent kexts?\n\n{}".format(
        ...
        for kext in ordered_kexts:
            ...
            if not temp_kext.get("Enabled", False):
                continue
            ...
            if temp_kext.get("BundlePath", "") in bundles_enabled + duplicate_bundles:
                ...
                info = next(
                    (x[1] for x in kext_list if x[0].get("BundlePath", "") == temp_kext.get("BundlePath", "")), None
                )
                if not info or not info.get("cfbi", None):
        ...
        if len(duplicate_bundles):
            if mb.askyesno(
                "Duplicate CFBundleIdentifiers",
                "Disable the following kexts with duplicate CFBundleIdentifiers?\n\n{}".format(
        ...
        if os.path.exists(oc_tools) and os.path.isdir(oc_tools):
                ...
                for name in files:
                    if not name.startswith(".") and name.lower().endswith(".efi"):
                        ...
                        new_tool_entry = {
                            ...
                            "Comment": name,
                            "Enabled": True,
                            "Path": os.path.join(path, name)[len(oc_tools) :]
                            .replace("\\", "/")
            ...
            if len(tools_duplicates):
                if mb.askyesno(
                    "Duplicate Tools",
                    "Disable the following Tools with duplicate Paths?\n\n{}".format("\n".join(tools_duplicates)),
                    parent=self,
                ):
        ...
        for path, subdirs, files in os.walk(oc_drivers):
            for name in files:
                if not name.startswith(".") and name.lower().endswith(".efi"):
                    ...
                    if not driver_add:
                        drivers_list.append(
                            os.path.join(path, name)[len(oc_drivers) :].replace("\\", "/").lstrip("/")
                        )  # Strip the /Volumes/EFI/
                    else:
                        new_driver_entry = {
                            # "Arguments": "",
                            "Enabled": True,
                            "Path": os.path.join(path, name)[len(oc_drivers) :]
                            .replace("\\", "/")
                            .lstrip("/"),  # Strip the /Volumes/EFI/
                        }
                        # Add our snapshot custom entries, if any - include the name of the .efi driver if the Comment
                        for x in driver_add:
                            new_driver_entry[x] = name if x.lower() == "comment" else driver_add[x]
                        drivers_list.append(
                            OrderedDict(sorted(new_driver_entry.items(), key=lambda x: str(x[0]).lower()))
                        )
        ...
        for driver in sorted(drivers_list, key=lambda x: x.get("Path", "").lower() if driver_add else x):
            if not driver_add:  # Old way
                if not isinstance(driver, basestring) or driver.lower() in [
                    x.lower() for x in drivers if isinstance(x, basestring)
                ]:
                    continue
            else:
        ...
        for driver in drivers:
            if not driver_add:  # Old way
                if not isinstance(driver, basestring) or not driver.lower() in [
                    x.lower() for x in drivers_list if isinstance(x, basestring)
                ]:
                    continue
            else:
        ...
        if len(drivers_duplicates):
            if mb.askyesno(
                "Duplicate Drivers",
                "Disable the following Drivers with duplicate Paths?\n\n{}".format("\n".join(drivers_duplicates)),
                parent=self,
            ):
        ...
        if self.controller.settings.get("force_snapshot_schema", False):
            ...
            for entries, values in (
                (tree_dict["ACPI"]["Add"], acpi_add),
                (tree_dict["Kernel"]["Add"], kext_add),
                (tree_dict["Misc"]["Tools"], tool_add),
                (tree_dict["UEFI"]["Drivers"], driver_add),
            ):
                values["Comment"] = ""
                values["Enabled"] = True
                if not values:
        ...
        for x in self._tree.get_children():
            undo_list.append({"type": "remove", "cell": x, "from": self._tree.parent(x), "index": self._tree.index(x)})
            self._tree.detach(x)
        # Finally, we add the nodes back
        self.add_node(tree_dict)
        # Add all the root items to our undo list
        for child in self._tree.get_children():
            undo_list.append({"type": "add", "cell": child})
    ...
    def _qualify_node(self, node, type_check):
        ...
        if isinstance(node, dict):  # Should be an undo/redo action
            if node.get("type") in ("move", "add"):
                return (None, None, None)  # Handled by the nodes loop
            task = node
            node = task.get("cell")
        if not node:
            return (None, None, None)  # Broken formatting?
        if task and task.get("type") == "edit":
            values = list(task.get("values", []))
            if len(values) < 2:
                return (None, None, None)  # Broken :(
        else:
            values = self.get_padded_values(node, 3)
    ...
    def _change_int_display(self, node_list, new_display="Decimal"):
        if not isinstance(node_list, (list, tuple, deque)):
            node_list = [node_list]
        for node in node_list:
            node, task, values = self._qualify_node(node, "number")
            if task == False or values is None:
                continue  # Didn't qualify
            value = values[1]
            ...
            if new_display.lower() == "hex":
                ...
                if sys.version_info < (3, 0):
                    value = base64.b64encode(
                        binascii.unhexlify(value.replace("<", "").replace(">", "").replace(" ", ""))
                    )
                else:
                    value = base64.b64encode(
                        binascii.unhexlify(value.replace("<", "").replace(">", "").replace(" ", "").encode("utf-8"))
                    ).decode("utf-8")
            values[1] = value
            if task and task.get("type") == "edit":
                task["values"] = values
    ...
    def reundo(self, event=None, undo=True, single_undo=None):
        ...
        for task in task_list[::-1]:
            ...
            if ttype == "edit":
                ...
                r_task_list.append(
                    {"type": "remove", "cell": cell, "from": self._tree.parent(cell), "index": self._tree.index(cell)}
                )
    ...
    def got_focus(self, event=None):
        ...
        if event and event.widget == self:
            self.lift()
            if (
                self.controller.settings.get("warn_if_modified", True)
                and self.current_plist
                and os.path.isfile(self.current_plist)
                and self.last_hash
            ):
    ...
    def move_selection(self, event):
        ...
        if target == self.get_root_node():
            return  # Nothing to do here as we can't drag it
        ...
        if not self.get_check_type(tv_item).lower() in ("dictionary", "array") or (
            len(self._tree.get_children(tv_item)) and not self._tree.item(tv_item, "open")
        ):
            # Keep it a sibling
        ...
        if self.drag_open is None:
            self.drag_open = self._tree.item(target, "open")
        if self._tree.item(target, "open"):
    ...
    def strip_comments(self, event=None):
        ...
        for node in nodes:
            if node == self.get_root_node():
                continue  # Can't strip the root node
            ...
            if any((x.startswith(prefix)) for x in names):
                # Found one, remove it
                removedlist.append(
                    {"type": "remove", "cell": node, "from": self._tree.parent(node), "index": self._tree.index(node)}
                )
    ...
    def strip_disabled(self, event=None):
        ...
        for node in nodes:
            ...
            if (
                check_type == "boolean"
                and (name == "enabled" and value == self.b_false())
                or (name == "disabled" and value == self.b_true())
            ):
    ...
    def save_plist_as(self, event=None, path=None):
        ...
        try:
            if binary:
                plist.dump(
                    plist_data, m, sort_keys=self.controller.settings.get("sort_dict", False), fmt=plist.FMT_BINARY
                )
                ...
                plist_text = self._format_data_string(
                    plist.dumps(plist_data, sort_keys=self.controller.settings.get("sort_dict", False))
                )
            ...
            if mem_hash != temp_hash:
                raise Exception(
                    "The in-memory and temp file hashes do not match.  Serializing to the temp directory failed."
                )
            if os.path.isfile(path):
                ...
                try:
                    os.utime(temp_file, (update_timestamp, update_timestamp))
                except:
                    pass
            # Copy the temp over
    ...
    def copy_selection(self, event=None):
        ...
        try:
            clipboard_string = plist.dumps(
                self.nodes_to_values(node), sort_keys=self.controller.settings.get("sort_dict", False)
            )
    ...
    def paste_selection(self, event=None):
        ...
        try:
            ...
            element_type = (
                "dict"
                if clip_check.startswith("<key>")
                else "array" if not clip_check.startswith("<array>") and len(clip_check.split("\n")) > 1 else None
            )
            if element_type:
            ...
            try:
                plist_data = plist.loads(
                    cb, dict_type=dict if self.controller.settings.get("sort_dict", False) else OrderedDict
                )
            except Exception as e:
        ...
        if plist_data is None:
            if len(clip):
                ...
                self.bell()
                mb.showerror(
                    "An Error Occurred While Pasting", "The pasted value is not a valid plist string.", parent=self
                )
            # Nothing to paste
            self.pasting_nodes = False
            return "break"
        ...
        index = 0
        if not node == "" and (
            not t in ("dictionary", "array") or (self._tree.get_children(node) and not self._tree.item(node, "open"))
        ):
            ...
            if node == self.get_root_node() and self.get_root_type() is None:
                ...
                if self.is_data(plist_data):
                    self._tree.item(
                        node,
                        values=(
                            self.get_type(plist_data),
                            self.get_data(plist_data),
                            "",
                        ),
                    )
                    ...
                    self._tree.item(
                        ...
                        values=(
                            self.get_type(plist_data),
                            plist_data.strftime("%b %d, %Y %I:%M:%S %p"),
                            "",
                        ),
                    )
                else:
    ...
    def _add_node(self, value, parentNode, key, check_binary):
        ...
        if isinstance(value, dict):
            if (
                (not check_binary or (check_binary and self.plist_type_string.get().lower() != "binary"))
                and len(value) == 1
                and "CF$UID" in value
                and isinstance(value["CF$UID"], (int, long))
                ...
                self._tree.item(
                    i,
                    values=(
                        self.get_type(value, override="UID"),
                        display_value,
                        "" if parentNode == "" else self.drag_code,
                    ),
                    tags=uid_tags,
                )
            else:
            ...
            and self.int_type_string.get().lower() == "hex"
        ):
            value = int(value)
            ...
            self._tree.item(
                ...
                values=(
                    self.get_type(value),
                    self.b_true() if value else self.b_false(),
                    "" if parentNode == "" else self.drag_code,
                ),
            )
    ...
    def find_node_by_uid_reference(self, uid_index):
        """
        Find a tree node by its UID index reference.
        In Apple's plist format, UIDs reference elements in the $objects array.
        """
        try:
            # Find the $objects array in the plist
            root_node = self.get_root_node()
            objects_node = None

            # Look for $objects key in the root dictionary
            for child in self._tree.get_children(root_node):
                if self._tree.item(child, "text") == "$objects":
                    objects_node = child
                    break
            ...
            # Get the children of the $objects array
            objects_children = self._tree.get_children(objects_node)

            # Check if the UID index is within bounds
            if uid_index < 0 or uid_index >= len(objects_children):
                return None

            # Return the node at the specified index
            return objects_children[uid_index]

        except:
            return None

    def get_uid_reference_display(self, uid_index):
        """
        Get the display text for a UID reference.
        For single values, displays the value in parentheses.
        For collections (dict/array), displays as a clickable reference.
        """
        try:
            ...
            # Check if the UID index is within bounds
            if uid_index < 0 or uid_index >= len(objects_children):
                return f"UID {uid_index} (invalid)"

            # Get the referenced node
            referenced_node = objects_children[uid_index]
            referenced_values = self._tree.item(referenced_node, "values")
            referenced_value = (
                referenced_values[1] if len(referenced_values) > 1 else referenced_values[0]
            )  # Get the actual value, not the type

            # Check if it's a collection (dict/array)
            children = self._tree.get_children(referenced_node)
            if children:  # Has children, so it's a collection
                # Count the number of items
                item_count = len(children)
                return f"UID {uid_index} → ({item_count} items)"
            else:
                # It's a single value, display it in parentheses
                return f"UID {uid_index} ({referenced_value})"

        except Exception as e:
            return f"UID {uid_index} (error: {str(e)})"

    def is_uid_clickable(self, uid_index):
        """
        Check if a UID reference is clickable (points to a collection).
        """
        try:
            # Find the $objects array
            root_node = self.get_root_node()
            ...
            # Check if it's a collection (dict/array) by examining the type
            referenced_values = self._tree.item(referenced_node, "values")
            if referenced_values and len(referenced_values) > 0:
                referenced_type = referenced_values[0].lower()
    ...
    def jump_to_uid_reference(self, uid_index):
        """
        Jump to the node referenced by a UID index.
        """
        try:
            referenced_node = self.find_node_by_uid_reference(uid_index)
            if referenced_node is not None:
                # Select and scroll to the referenced node
                self._tree.selection_set(referenced_node)
                self._tree.focus(referenced_node)
                self._tree.see(referenced_node)
                return True
        except:
            pass
        return False
    ...
    def is_data(self, value):
        return (sys.version_info >= (3, 0) and isinstance(value, bytes)) or (
            sys.version_info < (3, 0) and isinstance(value, plistlib.Data)
        )
    ...
    def new_row(self, target=None, force_sibling=False):
        ...
        if target == self.get_root_node() and not self.get_check_type(self.get_root_node()).lower() in (
            "array",
            "dictionary",
        ):
            self.adding_rows = False
        ...
        index = 0
        if (
            not self.get_check_type(target).lower() in ("dictionary", "array")
            or force_sibling
            or (not self._tree.item(target, "open") and len(self._tree.get_children(target)))
        ):
        ...
        new_cell = self._tree.insert(
            target,
            index,
            text=name,
            values=(
                self.menu_code + " String",
                "",
                self.drag_code,
            ),
        )
    ...
    def remove_row(self, target=None):
        ...
        self.add_undo({"type": "remove", "cell": target, "from": parent, "index": self._tree.index(target)})
    ...
    def set_type_by_index(self, index=None, menu=None, zero_based=False):
        ...
        if not valid_vals:
            return  # Menu has no commands
        if not zero_based:
            index -= 1  # original index started at 1, normalize to 0-based
        if index < 0 or index >= len(valid_vals):
            return  # Out of range
    ...
    def cycle_type(self, increment=True):
        ...
        if not valid_vals:
            return  # Menu has no commands
        try:
            curr = valid_vals.index(value)
        except:
            return  # Our current value isn't in the menu?
        mod = 1 if increment else -1
    ...
    def change_type(self, value, cell=None):
        ...
        for i in self._tree.get_children(cell):
            changes.append({"type": "remove", "cell": i, "from": self._tree.parent(i), "index": self._tree.index(i)})
            self._tree.detach(i)
        changes.append(
            {
                "type": "edit",
    ...
    def set_bool(self, value):
        ...
        if values[1] == value:
            return  # Nothing to do, setting it to itself.
        cell = "" if not len(self._tree.selection()) else self._tree.selection()[0]
        self.add_undo(
            {"type": "edit", "cell": cell, "text": self._tree.item(cell, "text"), "values": [x for x in values]}
        )
        values[1] = value
        # Set the values
    ...
    def merge_menu_preset(self, val=None):
        ...
        for p, t in izip(paths, types):
                    ...
                    if not current_type.lower() == needed_type.lower():
                        ...
                        if mb.askyesno(
                            ...
                            "{} is {}, should be {}.\n\nWould you like to replace it?".format(
                                cell_name, current_type, needed_type
                            ),
                            parent=self,
                        ):
            ...
            if not found:
                # Need to add it
                current_cell = self._tree.insert(
                    current_cell,
                    tk.END,
                    text=p,
                    values=(
                        self.menu_code + " " + needed_type,
                        "",
                        self.drag_code,
                    ),
                    open=True,
                )
                undo_list.append({"type": "add", "cell": current_cell})
        ...
        if current_type == "dictionary":
            ...
            for x in self._tree.get_children(current_cell):
                ...
                if name in value:
                    ...
                    undo_list.append({"type": "remove", "cell": x, "from": current_cell, "index": self._tree.index(x)})
                    self._tree.detach(x)
            # Add the entries
            if isinstance(value, dict):
                just_add = False
                for x in value:
                    ...
                    undo_list.append({"type": "add", "cell": created})
        if just_add:
            last_cell = self.add_node(value, current_cell, "")
            undo_list.append({"type": "add", "cell": last_cell})
        self.add_undo(undo_list)
        self._ensure_edited()
        self.update_all_children()
        self.alternate_colors()
    ...
    def do_sort(self, cell, recursive=False, reverse=False):
        ...
        while node_stack:
            ...
            for index, child in enumerate(sorted_children):
                ...
                if not skip_sort:
                    undo_tasks.append(
                        {"type": "move", "cell": child, "from": node, "to": node, "index": self._tree.index(child)}
                    )
    ...
    def popup(self, event):
        ...
        if cell:
            self.select(cell, alternate=False)
        ...
        if cell in ("", self.get_root_node()):
            ...
            if self.get_check_type(self.get_root_node()).lower() in ("array", "dictionary"):
                popup_menu.add_command(
                    label="New top level entry{}".format(" (Cmd +)" if is_mac else ""),
                    command=lambda: self.new_row(self.get_root_node()),
            ...
            if self.get_check_type(cell).lower() in ("dictionary", "array") and (
                ...
                popup_menu.add_command(
                    label="New sibling of '{}'{}".format(self._tree.item(cell, "text"), " (Cmd +)" if is_mac else ""),
                    command=lambda: self.new_row(cell),
                    accelerator=None if is_mac else "(Ctrl +)",
                )
                popup_menu.add_command(
                    label="Remove '{}'{}".format(self._tree.item(cell, "text"), " (Cmd -)" if is_mac else ""),
                    command=lambda: self.remove_row(cell),
                    accelerator=None if is_mac else "(Ctrl -)",
                )
        ...
        if self.get_check_type(parent).lower() in ("dictionary", "array"):
            ...
            recurs_target = (
                cell
                if self.get_check_type(cell).lower() in ("dictionary", "array") and len(self._tree.get_children(cell))
                else parent
            )
            popup_menu.add_command(
                label="Recursively sort keys starting at '{}'".format(self._tree.item(recurs_target, "text")),
                command=lambda: self.sort_keys(recurs_target, recursive=True),
            )
            popup_menu.add_command(
                label="Recursively reverse sort keys starting at '{}'".format(self._tree.item(recurs_target, "text")),
                command=lambda: self.sort_keys(recurs_target, recursive=True, reverse=True),
            )
            # Check the actual cell
            sort_target = (
                cell
                if cell in ("", self.get_root_node())
                or (self.get_check_type(cell).lower() == "dictionary" and len(self._tree.get_children(cell)) > 1)
                else self._tree.parent(cell)
            )
            popup_menu.add_command(
                label="Sort keys in '{}'".format(self._tree.item(sort_target, "text")),
                command=lambda: self.sort_keys(sort_target),
            )
            popup_menu.add_command(
                label="Reverse sort keys in '{}'".format(self._tree.item(sort_target, "text")),
                command=lambda: self.sort_keys(sort_target, reverse=True),
            )
        ...
        try:
            ...
            p_state = "disabled"  # Invalid clipboard content
        popup_menu.add_command(
            label="Copy{}".format(" (Cmd+C)" if is_mac else ""),
            command=self.copy_selection,
            state=c_state,
            accelerator=None if is_mac else "(Ctrl+C)",
        )
        if not cell in ("", self.get_root_node()) and self.get_check_type(cell).lower() in ("array", "dictionary"):
            popup_menu.add_command(
                label="Copy Children{}".format(" (Cmd+Shift+C)" if is_mac else ""),
                command=self.copy_children,
                state=c_state,
                accelerator=None if is_mac else "(Ctrl+Shift+C)",
            )
        popup_menu.add_command(
            label="Paste{}".format(" (Cmd+V)" if is_mac else ""),
            command=self.paste_selection,
            state=p_state,
            accelerator=None if is_mac else "(Ctrl+V)",
        )
        cell_path = self.get_cell_path(cell)
        # Add rbits option
        cell_search = [x for x in self.split(cell_path) if x and x != "*"]
        if cell_search and cell_search[0] == "Root":
            cell_search = cell_search[1:]
        if cell_search and os.path.isfile(self.controller.get_best_tex_path()):
            popup_menu.add_separator()
            popup_menu.add_command(
                label='Show info for "{}"{}'.format(" -> ".join(cell_search), " (Cmd+I)" if is_mac else ""),
                command=self.show_config_info,
                accelerator=None if is_mac else "(Ctrl+I)",
            )
        ...
        for key in sorted(list(self.menu_data)):
            ...
            for item in sorted(valid):
                ...
                option_menu.add_cascade(
                    label=" -> ".join(parts[1 if parts[0].lower() == "root" and len(parts) > 1 else 0 :]),
                    menu=item_menu,
                )
    ...
    def on_double_click(self, event):
        ...
        if sys.platform == "darwin":
            ...
            ctrl_pressed = (event.state & 0x10000) != 0 or (event.state & 0x4) != 0
        else:
            # Windows/Linux: Check for Control key (0x4)
        ...
        if index == 1:
            ...
            return "break"
        if parent == "" and (
            index == 0 or self.get_check_type(self.get_root_node()).lower() in ("array", "dictionary")
        ):
            return "break"  # Not changing the type - can't change the name of Root
        if index == 2:
            if t and t.lower() in ("dictionary", "array"):
    ...
    def get_root_node(self):
        ...
        if not len(children) == 1:
            raise Exception("Root is malformed!")
        return children[0]
    ...
    def set_colors(self, event=None, alternate=False):
        ...
        # Configure UID tags for visual indication
        # Create underlined font for clickable UIDs
        try:
            if sys.version_info < (3, 0):
                import tkFont
                ...
                base_font = tkFont.nametofont("TkDefaultFont")
            underlined_font = base_font.copy()
            underlined_font.configure(underline=True)
            self._tree.tag_configure("uid_clickable", foreground="blue", font=underlined_font)
            self._tree.tag_configure("uid_non_clickable", foreground=self.r1t, background=self.r1)
        except:
            # Fallback if font configuration fails
            self._tree.tag_configure("uid_clickable", foreground="blue")
            self._tree.tag_configure("uid_non_clickable", foreground=self.r1t, background=self.r1)
        if alternate:
            self.alternate_colors()
    ...
    def alternate_colors(self, event=None, start_with=None):
        ...
        for x, item in enumerate(visible):
            ...
            if start_with is None or found_start:
                ...
                current_tags = self._tree.item(item, "tags")
                # Preserve UID tags while updating alternating colors
                uid_tags = [tag for tag in current_tags if tag.startswith("uid_")]
                color_tag = sel if item == focus else odd if x % 2 else even
                new_tags = uid_tags + list(color_tag)
                if current_tags != tuple(new_tags):
    ...
    def show_config_info(self, event=None):
        ...
        window = next(
            (
                x
                for x in self.controller.stackorder(self.controller.tk, include_defaults=True)
                if x.title() == check_title
            ),
            None,
        )
        if not window:
            ...
            if window:
                ...
                window.bind(
                    "<{}-w>".format("Command" if sys.platform == "darwin" else "Control"), self.controller.close_window
                )
